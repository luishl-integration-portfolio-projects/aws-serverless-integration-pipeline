...
try {
    ...
    if ($wslIp) { $m = [regex]::Match($wslIp, 'inet (\d+\.\d+\.\d+\.\d+)'); if ($m.Success) { $candidates += "http://$($m.Groups[1].Value):4566" } }
} catch {}
foreach ($uri in $candidates) {
    try { $null = Invoke-RestMethod -Uri "$uri/_localstack/health" -ErrorAction Stop -TimeoutSec 2; $lsUri = $uri; break } catch { continue }
}
if (-not $lsUri) {
...
if ($exists) {
    ...
    $output = & podman $podmanBase lambda update-function-code --function-name $functionName --zip-file fileb:///workspace/funcion_lambda.zip 2>&1 | Out-String
    $exitCode = $LASTEXITCODE
} else {
    Write-Host "  -> Function does not exist. Creating new function..." -ForegroundColor Cyan
    $output = & podman $podmanBase lambda create-function --function-name $functionName --runtime python3.12 --role arn:aws:iam::000000000000:role/lambda-ex --handler index.lambda_handler --zip-file fileb:///workspace/funcion_lambda.zip 2>&1 | Out-String
    $exitCode = $LASTEXITCODE
}
...
if ($exitCode -eq 0 -and ($output -match '"FunctionArn"' -or $output -match '"LastModified"')) {
    ...
    # Wait for function to become Active (LocalStack creates functions asynchronously)
